// ---------------------------------------------------------------------------
// CustodianteTable: paginated table for Custodiante (T-5)
// ---------------------------------------------------------------------------
...
interface CustodianteTableProps {
  items: CustodianteDto[];
  isLoading: boolean;
  canWrite: boolean;
  onEdit: (item: CustodianteDto) => void;
}

export function CustodianteTable({
  items,
  isLoading,
  canWrite,
  onEdit,
}: CustodianteTableProps) {
  if (isLoading) {
    return (
      <div className="space-y-2" aria-busy="true" aria-label="Carregando custodiantes">
            ...
            {canWrite && (
              <TableCell>
                <Button
                  ...
                  size="icon"
                  onClick={() => onEdit(item)}
                  aria-label={`Editar custodiante ${item.razaoSocial}`}
                >
