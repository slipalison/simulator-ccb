/**
 * T-9b: Backoffice SPA api-proxy regression spec (Phase 49 iter 2 — auth-flow-fix)
 *
 * Mirror of frontend/client/playwright/specs/api-proxy.spec.ts for port 5174.
 * Covers three scenarios guarding against Bug 3 (api-proxy 503 TypeError: fetch failed)
 * documented in .jdi/phases/auth-flow-fix/INVESTIGATION-api-proxy.md.
 *
 * Root cause: a stale vinxi-host process on the Windows host intercepts `localhost:5174`
 * via IPv6 ([::]:5174), cannot reach the Docker bridge `onboarding-net`, and returns a
 * 503 HTML error page instead of proxying to the .NET backend. These tests use
 * `http://127.0.0.1:5174` (D-17) to ensure IPv4 routing to the Docker port mapping.
 *
 ...
 *   Shells out to detect listeners on port 5174. Asserts exactly ONE process listens.
 ...
 *   Both SPAs proxy /api/* to the same backend — registration endpoint is unauthenticated
 *   and returns 422 on empty body (FluentValidation) regardless of which SPA port is used.
...
function probeListeners(port: number): Array<{ pid: number }> | null {
  if (process.platform === 'win32') {
    ...
    try {
      ...
      // ss output lines: "LISTEN 0 128 0.0.0.0:5174 ... users:(("node",pid=456,fd=4))"
      const pids: number[] = [];
...
test.describe('api-proxy — backoffice SPA (port 5174)', () => {
  test('Scenario 1 — single listener guard: exactly one process listens on port 5174', () => {
  ...
  test('Scenario 2 — proxy reaches backend on POST: /api/companies/registration returns 422 JSON (not 503 HTML)', async ({
    ...
    // Both SPAs proxy /api/* to the same backend (http://api:8080). The registration
    // endpoint is unauthenticated, making it the ideal smoke test for the proxy layer
    // without requiring an active admin session.
