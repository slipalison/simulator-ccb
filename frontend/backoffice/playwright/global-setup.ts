 ...
 * WSL). In CI (Linux), bash is available directly. No conditional branching
 * needed — Node spawns bash in both environments when Git for Windows is on PATH.
 *
 * If docker compose is not available or the stack is not healthy, the suite
 * aborts with an actionable error message instead of running and failing
 * inside test cases (which would produce confusing output).
 */
export default async function globalSetup(): Promise<void> {
  ...
  try {
    ...
    // If parsing fails, fall through — we'll check the raw output string
    services = [];
  }
  ...
  for (const svc of requiredServices) {
    ...
    // State is "running" for healthy services; anything else is a problem
  ...
  try {
    ...
    // Print seed output so CI logs show what happened
