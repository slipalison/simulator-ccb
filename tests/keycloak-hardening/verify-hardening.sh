...
# Resolve Python — support python3 (Linux/macOS) and python (Windows/some Linux)
if command -v python3 >/dev/null 2>&1 && python3 -c "import sys; sys.exit(0)" 2>/dev/null; then
  ...
  echo "ERROR: Python 3 not found. Install python3 or python and retry."
  exit 1
fi
...
TOKEN=$(curl -s -X POST "$KC_BASE/realms/master/protocol/openid-connect/token" \
  ...
  | $PYTHON -c "import json,sys; d=json.load(sys.stdin); print(d.get('access_token','FAILED'))")
...
BF=$(echo "$REALM" | $PYTHON -c "import json,sys; d=json.load(sys.stdin); print(d.get('bruteForceProtected'))")
FF=$(echo "$REALM" | $PYTHON -c "import json,sys; d=json.load(sys.stdin); print(d.get('failureFactor'))")
WI=$(echo "$REALM" | $PYTHON -c "import json,sys; d=json.load(sys.stdin); print(d.get('waitIncrementSeconds'))")
[ "$BF" = "True" ] && [ "$FF" = "5" ] && [ "$WI" = "30" ] \
...
PP=$(echo "$REALM" | $PYTHON -c "import json,sys; d=json.load(sys.stdin); print(d.get('passwordPolicy',''))")
...
# Exclude Keycloak built-in system clients (account, account-console, security-admin-console, broker, realm-management, admin-cli)
CLIENTS=$(curl -s -H "Authorization: Bearer $TOKEN" "$KC_BASE/admin/realms/$KC_REALM/clients")
WILDCARD_COUNT=$(echo "$CLIENTS" | $PYTHON -c "
import json,sys
SYSTEM_CLIENTS = {'account', 'account-console', 'security-admin-console', 'broker', 'realm-management', 'admin-cli'}
clients = json.load(sys.stdin)
count = sum(1 for c in clients if c.get('clientId') not in SYSTEM_CLIENTS for uri in c.get('redirectUris', []) if '*' in uri)
print(count)
")
[ "$WILDCARD_COUNT" = "0" ] \
  && log_pass "SEC-03: No wildcard redirect URIs found in application clients" \
  || log_fail "SEC-03: Found $WILDCARD_COUNT wildcard redirect URIs in application clients"
...
SSL=$(echo "$REALM" | $PYTHON -c "import json,sys; d=json.load(sys.stdin); print(d.get('sslRequired'))")
[ "$SSL" = "external" ] \
...
CLIENT_INT_ID=$(echo "$CLIENTS" | $PYTHON -c "
import json,sys
clients = json.load(sys.stdin)
for c in clients:
...
EXTRA_ROLES=$(echo "$ROLE_MAPPINGS" | $PYTHON -c "
import json,sys
